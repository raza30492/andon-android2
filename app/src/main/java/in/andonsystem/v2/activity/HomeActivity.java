...
public class HomeActivity extends AppCompatActivity
    ...
    protected void onStart() {
        super.onStart();
        List<Buyer> buyers = buyerService.findByTeam("TL-Amit");
        StringBuilder builder = new StringBuilder();
        builder.append("Buyers: \n");
        //builder.append(buyerService.findOne(1L).getName() + "\n");
        for (Buyer buyer: buyers){
            builder.append(buyer.getName() + "\n");
        }
        builder.append("\nTeams: \n");
        for (String str: app.getTeams()){
            builder.append(str + "\n");
        }
        builder.append("\nProblems: \n");
        for (String str: app.getProblems()){
            builder.append(str + "\n");
        }
        User user = new UserService(app).findOne(1L);
        if(user != null) builder.append("\nUser: \n" + user.getName());
